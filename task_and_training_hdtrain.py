...
hyperparameters = {
    ...
    "pretrain_for_steps": 10000,
    "save_network_every_steps": 10000,
    "note_error_every_steps": 200,  # only relevant if verbose is True
...
additional_comments = [
    ...
    "Pre-trained on the 2ORI2O task"
]
...
class Task_pretrain:
    ...
    def _input_orientation_representation(orientation):
        ...
        rates = Task_pretrain._o_spikes(pref, stim, exponent, max_spike, k)[0]
        return rates
    ...
    def _make_trial(orientation1, orientation2, delay0, delay1, delay2):
        ...
        o_beforecue = torch.zeros(task_parameters["show_orientation_for"] * 2 + delay0 + delay1 + delay2, task_parameters["dim_output"]+2)
        o_cue = Task_pretrain._output_orientation_representation(orientation1, orientation2).repeat(task_parameters["show_cue_for"], 1)
    ...
    def make_random_orientations_batch(batch_size, delay0, delay1, delay2):
        ...
        for j in range(batch_size):
            orientation1, orientation2, *_ = Task_pretrain.choose_trial_parameters(None, None, delay0, delay1, delay2)
            i_full, o_full, b_mask = Task_pretrain._make_trial(orientation1, orientation2, delay0, delay1, delay2)
...
class Model_pretrain(torch.nn.Module):
    def __init__(self):
        ...
        dim_output = model_parameters["dim_output"]+2
...
def train_network_pretrain(model):
    ...
    max_steps = hyperparameters["pretrain_for_steps"]
    ...
    for p in range(max_steps + 1):
        _, _, delay0, delay1, delay2 = Task_pretrain.choose_trial_parameters()  # choose the delays for this batch
        input, target, output_mask = Task_pretrain.make_random_orientations_batch(batch_size, delay0, delay1, delay2)
        noise_mask = Task_pretrain.get_noise_mask(delay0, delay1, delay2)
...
if __name__ == "__main__":
    ...
    # pretrain the network and save weights
    model_p = Model_pretrain()
    result = train_network_pretrain(model_p)
    # train the network and save weights
    model = Model()
    with torch.no_grad():
        model.fc_h2ah.weight = nn.Parameter(model_p.fc_h2ah.weight.detach().clone())
        model.fc_x2ah.weight = nn.Parameter(model_p.fc_x2ah.weight.detach().clone())
        if model_parameters["input_bias"]:
            model.fc_x2ah.bias = nn.Parameter(model_p.fc_x2ah.bias.detach().clone())
        model.fc_h2y.weight = nn.Parameter(model_p.fc_h2y.weight[:2].detach().clone())
        if model_parameters["output_bias"]:
            model.fc_h2y.bias = nn.Parameter(model_p.fc_h2y.bias[:2].detach().clone())
