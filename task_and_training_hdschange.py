...
parser.add_argument('--shift_amount', type=float, help='how much to shift tunings?', default=60)
...
additional_comments = [
    ...
    "Structured change of tuning network, training is on top-level parameters + output layer"
]
...
directory += f"_sa{model_parameters['shift_amount']}"
...
R1_pref_changes = []
R1_pref_changes += [-model_parameters["shift_amount"] for _ in range(len(R1_i)//2)]
R1_pref_changes += [model_parameters["shift_amount"] for _ in range(len(R1_i)//2)]
...
class Model(torch.nn.Module):
    ...
    def forward(self, input, noise):
        ...
        self.W_h_ah = _legi(self.R1_pref.repeat(len(self.R1_pref), 1), self.R1_pref.repeat(len(self.R1_pref), 1).T) * self.top_parameters[0]
        self.W_x_ah = _legi((self.R1_pref+self.R1_pref_changes).repeat(task_parameters["input_orientation_units"], 1).T, self.IN_pref.repeat(len(R1_pref), 1)) * self.top_parameters[0]
