...
additional_comments = [
    ...
    "Inversion of tuning network, training is on top-level parameters + output layer"
]
...
R1a_pref = R1a_i/len(R1a_i)*180
R1b_pref = R1a_i/len(R1a_i)*180
...
class Model(torch.nn.Module):
    ...
    def forward(self, input, noise):
        ...
        W_h_ah = torch.zeros_like(self.W_h_ah, dtype=torch.float32)
        W_x_ah = torch.zeros_like(self.W_x_ah, dtype=torch.float32)
        W_h_ah[R1a_si:R1a_ei, R1a_si:R1a_ei] = _legi(self.R1a_pref.repeat(len(self.R1a_pref), 1), self.R1a_pref.repeat(len(self.R1a_pref), 1).T) * self.top_parameters[0]
        W_h_ah[R1b_si:R1b_ei, R1b_si:R1b_ei] = _legi(self.R1b_pref.repeat(len(self.R1b_pref), 1), self.R1b_pref.repeat(len(self.R1b_pref), 1).T) * self.top_parameters[0]
        W_h_ah[R1b_si:R1b_ei, R1a_si:R1a_ei] = _legi(self.R1b_pref.repeat(len(self.R1a_pref), 1), self.R1a_pref.repeat(len(self.R1b_pref), 1).T) * (-self.top_parameters[0]-self.top_parameters[2])
        W_h_ah[R1a_si:R1a_ei, R1b_si:R1b_ei] = _legi(self.R1a_pref.repeat(len(self.R1b_pref), 1), self.R1b_pref.repeat(len(self.R1a_pref), 1).T) * (-self.top_parameters[0]+self.top_parameters[2])
        W_x_ah[R1a_si:R1a_ei, :-1] = _legi(self.R1a_pref.repeat(task_parameters["input_orientation_units"], 1).T, self.IN_pref.repeat(len(self.R1a_pref), 1)) * self.top_parameters[0]
        W_x_ah[R1b_si:R1b_ei, :-1] = _legi(self.R1b_pref.repeat(task_parameters["input_orientation_units"], 1).T, self.IN_pref.repeat(len(self.R1b_pref), 1)) * self.top_parameters[0]
        self.W_h_ah = W_h_ah
        self.W_x_ah = W_x_ah
        self.b_ah = torch.ones_like(self.b_ah) * self.top_parameters[1]
        #im = plt.imshow(W_h_ah.detach())
        #im = plt.imshow(_legi(self.R1a_pref.repeat(len(self.R1a_pref), 1), self.R1a_pref.repeat(len(self.R1a_pref), 1).T) * self.top_parameters[0].detach())
        #plt.colorbar(im)
        #plt.show()
        #exit()
...
if __name__ == "__main__":
    ...
    error_store, error_store_o1, error_store_o2, gradient_norm_store = result["error_store"], result["error_store_o1"], result["error_store_o2"], result["gradient_norm_store"]
