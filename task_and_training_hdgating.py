...
R1_pref = R1_i/len(R1_i)*180
DT_pref = DT_i/len(DT_i)*180
...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        # 4: R1 bias, DT bias
        # 6: R1-DT nonspecific connection magnitude
    ...
    def forward(self, input, noise):
        ...
        W_h_ah[R1_si:R1_ei, R1_si:R1_ei] = _legi(self.R1_pref.repeat(len(self.R1_pref), 1), self.R1_pref.repeat(len(self.R1_pref), 1).T) * self.top_parameters[0]
        W_h_ah[R1_si:R1_ei, DT_si:DT_ei] = _legi(self.R1_pref.repeat(len(self.DT_pref), 1), self.DT_pref.repeat(len(self.R1_pref), 1).T) * self.top_parameters[0]
        W_h_ah[DT_si:DT_ei, R1_si:R1_ei] = torch.ones((len(DT_i), len(R1_i))) * self.top_parameters[2]
        W_x_ah[DT_si:DT_ei, :-1] = _legi(self.DT_pref.repeat(task_parameters["input_orientation_units"], 1).T, self.IN_pref.repeat(len(self.DT_pref), 1)) * self.top_parameters[0]
